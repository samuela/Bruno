...
function bindKeyToName(key, mask, name) {
	if (mask.equals("command")) {
		bruno.getRootPane().getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(
				KeyStroke.getKeyStroke(key, Toolkit.getDefaultToolkit()
						.getMenuShortcutKeyMask()), name);
	} else if (mask.equals("control")) {
		bruno.getRootPane().getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(
				KeyStroke.getKeyStroke(key, ActionEvent.CTRL_MASK), name);
	}
}
