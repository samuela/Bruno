...
public class EditHistoryView extends JPanel {
	...
	/*
	 * public void revert(NodeComponent node) { Component[] nodeComponents =
	 * nodesView.getComponents(); int difference = 0; for (int
	 * i=nodeComponents.length-1; i>=0; i--){ Component component =
	 * nodeComponents[i]; if (component == node){ if
	 * (i==nodeComponents.length-1) return; else{ difference =
	 * nodeComponents.length-1-i; break; } } else{ ((NodeComponent)
	 * component).makeOppositeComponent(this); } } nodeComponents =
	 * nodesView.getComponents(); addCompoundNode((NodeComponent)
	 * nodeComponents[nodeComponents.length-difference], (NodeComponent)
	 * nodeComponents[nodeComponents.length-1], "Revert"); }
	 */

	/*
	 * public void addCompoundNode(CompoundEdit compound) { Edit top =
	 * compound.getTop(); Edit bottom = compound.getBottom(); int index = 0;
	 * NodeComponent topNode = nodes.get(index); while (topNode.getEdit() !=
	 * top){ index++; topNode = nodes.get(index); } NodeComponent newNode = new
	 * NodeComponent(compound, undoController); nodesView.add(newNode, index);
	 * nodes.add(index, newNode); revalidate(); index++; NodeComponent toRemove
	 * = topNode; while (toRemove.getEdit() != bottom){ nodesView.remove(index);
	 * revalidate(); nodes.remove(index); toRemove = nodes.get(index); }
	 * nodesView.remove(toRemove); revalidate(); }
	 */
