...
public class EditHistoryView extends JPanel {
    ...
    private PlaceholderTextField comment;
	...
	comment = new PlaceholderTextField();
	comment.setPlaceholderText("Edit message...");
	comment.setEditable(false);
	comment.getDocument().addDocumentListener(new MyDocumentListener());

	final JSplitPane rightBottom = new JSplitPane(
						      JSplitPane.VERTICAL_SPLIT, comment, sp);
	// Because fuck Swing.
	SwingUtilities.invokeLater(new Runnable() {

		@Override
		    public void run() {
		    rightBottom.setDividerLocation((int) comment.getPreferredSize()
						   .getHeight());
		}
	    });
	...
	expandNode.addActionListener(new ActionListener() {
		@Override
			...
			expand(selectedNode);
			expandNode
			    .setEnabled(selectedNode.getEdit().getCanExpand());
		    }
		}
	    });
    ...
    /**
     * Add a new edit to the edit history view. If the scrollbar is already at
     * the bottom keep it there otherwise don't move it.
     */
	...
	boolean atBottom = (currentValue == scrollBar.getMaximum()
	...
	if (atBottom) {
	    javax.swing.SwingUtilities.invokeLater(new Runnable() {
		    @Override
			public void run() {
			JScrollBar scrollBar = nodesViewScrollPane
    ...
    /**
     * Adds an edit to the beginning. This is used only in deserialization of an
     * UndoController.
     */
	...
	CompoundEdit maskingEdit = ((NodeComponent) nodeComponents[higher])
	    .getEdit();
	Mask mask = new Mask(maskingEdit);
	((NodeComponent) nodeComponents[higher]).setColor();
	for (int i = higher; i > lower; i--) {
	    CompoundEdit toRemove = ((NodeComponent) nodesView
				     .getComponent(lower)).getEdit();
	...
	if (node.getEdit().getIsMask()) {
	    ...
	    for (CompoundEdit edit : Lists.reverse(node.getEdit().getLastMask()
						   .getMaskedEdits())) {
    ...
    public void showEdit(CompoundEdit edit)
    {
	Component[] nodeComponents = nodesView.getComponents();
	NodeComponent component = null;
	for (int i = 0; i < nodeComponents.length; i++) {
	    if (((NodeComponent) nodeComponents[i]).getEdit() == edit) {
		component = (NodeComponent) nodeComponents[i];
		break;
	    }
	}
	if (component != null)
