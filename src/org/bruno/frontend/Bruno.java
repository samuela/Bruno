...
public class Bruno extends JFrame {
    ...
    public static final String SUPPORT_DIR = System.getProperty("user.home")+"/Library/Application Support/Bruno";
	...
	getRootPane().getActionMap().put("foobar", new AbstractAction() {
		...
		@Override
			...
			&& getFocusOwner().equals(foobar.getField())) {
     ...
     * ClassNotFoundException { File rootFolder =
     * getProjectExplorer().getCurrentFolder(); process(rootFolder, comment); }
     * 
     * private void process(File file, String comment) throws IOException,
     * ClassNotFoundException { if (file.isFile()) { DocumentModel doc1 = new
     * DocumentModel(file); if (doc1.getMetadataFile().exists()) {
     * System.out.println(file); EditingWindow ew = null; DocumentModel doc2 =
     * getEditingWindow().getDoc(); if (doc2 != null &&
     * !doc2.getFile().equals(file)) { ew = new EditingWindow(null, doc2); }
     * else { ew = getEditingWindow(); }
     * 
     * // Do the revert ew.getUndoController().revertByComment(comment); // Save
     * ew.save(); } } else { File[] fileList = file.listFiles(); for (int i = 0;
     * i < fileList.length; i++) { process(fileList[i], comment); } } }
     */

    public AbstractAction scriptAction(final String scriptName) {
	return new AbstractAction() {
	    private static final long serialVersionUID = 1L;

	    @Override
		public void actionPerformed(ActionEvent e) {
		if (pluginManager.supportsScript(scriptName)) {
		    pluginManager.executeScript(scriptName);
		} else {
		    // fail silently - script not supported
		}
	    }
	};
    }
